...
void SQLiteCustomerDAO::addCustomer(Customer* customer)
{
    std::string sql = "INSERT INTO Customers (name, walletBalance, password, loyaltyPoints, membershipLevel, upgradeMessageShown) VALUES ('" +
                      ...
                      DatabaseManager::escapeSql(customer->getPassword()) + "', 0, 'Normal', 0);";
...
{
    std::string sql = "SELECT id, name, walletBalance, password, loyaltyPoints, membershipLevel, badge, monthlyCoupons, upgradeMessageShown FROM Customers WHERE id = " + std::to_string(id) + ";";
    ...
    try {
        upgradeMessageShown = std::stoi(row.at("upgradeMessageShown"));
    } catch (...) { upgradeMessageShown = 0; }
    ...
    c->setUpgradeMessageShown(upgradeMessageShown);
    return c;
}
...
std::vector<Customer*> SQLiteCustomerDAO::getAllCustomers() const
{
    ...
    std::string sql = "SELECT id, name, walletBalance, password, loyaltyPoints, membershipLevel, badge, monthlyCoupons, upgradeMessageShown FROM Customers;";
